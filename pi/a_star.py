...
class AStar:
  ...
  def play_notes(self, notes):
    total_len = len(notes)
    if total_len > self.max_notes:
      print(f"we only support {self.max_notes}, but you pass a note with {total_len} bytes")
      exit()
    print(f'{notes.encode("ascii")}')
    self.write_pack(24, f'B{total_len + 1}s', 1, notes.encode("ascii"))
